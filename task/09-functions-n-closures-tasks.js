...
function getPolynom() {
    return !arguments.length ? null :
        (x) => Array.from(arguments).reduce((prev, el, i) => prev + el * Math.pow(x, arguments.length - 1 - i), 0);
}
...
function retry(func, attempts) {
    return function () {
        ...
        while (tryCounter < attempts) {
            try {
                return func();
                //if need more attemps -> go recoursive
            } catch (e) {
...
function logger(func, logFunc) {
    return function() {
        ...
        //trick from google with json.stringify

        logFunc(`${funcName} starts`);
        let result = func.apply(this, arguments);
        logFunc(`${funcName} ends`);
        //eazy-eazy with template strings to access variable
